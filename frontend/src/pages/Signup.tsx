    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex flex-col justify-center flex-1 p-12">
        <div className="max-w-sm w-full mx-auto">
          ...
          <p className="text-sm text-gray-500 mb-8">Start writing your thoughts today</p>
          {error && <p className="text-red-500 text-sm mb-4">{error}</p>}
          <div className="space-y-4">
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Name</label>
              <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-400" placeholder="Lalit" value={name} onChange={e => setName(e.target.value)} />
            </div>
            <div>
            ...
            <button onClick={handleSignup} className="w-full bg-[#7F77DD] text-white rounded-lg py-2 text-sm font-medium hover:bg-[#6c64c9] transition-colors">
              Sign up
            </button>
          </div>
          <p className="text-center text-sm text-gray-500 mt-6">
            Already have an account?{" "}
            <Link to="/login" className="text-[#7F77DD]">Login</Link>
          </p>
        </div>
      </div>
    </div>
